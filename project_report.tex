...
Throughout, we emphasize interpretability and robustness: results are reported on a log scale when appropriate, effect sizes are paired with 95\% confidence intervals, and key conclusions are challenged with at least one robustness check.
...
We find that billionaire wealth is extremely right-skewed, even after restricting the sample to ultra-high-net-worth individuals. 
On the log scale, wealth is positively associated with country-level GDP, though the relationship explains only part of the variation and remains strictly non-causal. 
Group comparisons suggest statistically detectable but practically modest differences between self-made and non–self-made individuals. 
Importantly, these qualitative conclusions remain stable under alternative specifications and after excluding the most extreme observations.
...
We treat the raw CSV as initial input and record a ``schema freeze'' (column set, read rules, and file fingerprint). This prevents analysis drift and ensures that future reruns of the notebook reproduce the same artifacts.

\paragraph{Missingness is part of the signal.}
We define the missingness rate of a variable $X$ as
\[
\mathrm{MissingRate}(X)=\frac{\#\{i:\,X_i \text{ is empty}\}}{n}.
\]
...
We start from the original dataset of 2,640 billionaires with 35 variables and enrich it through feature engineering without removing any observations. 
Six new variables are added: standardized industry category (\texttt{category\_canonical}), binary indicators for U.S. residency (\texttt{is\_us}) and self-made status (\texttt{selfMade\_bool}), 
country-level GDP (\texttt{gdp\_country\_num}), and their log-transformed counterparts (\texttt{log\_finalWorth}, \texttt{log\_gdp}). The final cleaned dataset thus contains 41 columns while preserving all 2,640 records.

Following preprocessing and feature engineering, the cleaned dataset \texttt{billionaires\_clean.csv} was saved to \texttt{data/clean/} for reproducibility, preserving all 2,640 original records while expanding the feature set from 35 to 41 variables. The additional columns include standardized categorical labels (\texttt{category\_canonical}), binary flags (\texttt{is\_us}, \texttt{selfMade\_bool}), country-level GDP (\texttt{gdp\_country\_num}), and log-transformed wealth and economic indicators (\texttt{log\_finalWorth}, \texttt{log\_gdp}). Dataset metadata (file path, dimensions) were recorded in manifest table~\ref{tab:tab_M3_clean_dataset_manifest}.
...
\label{tab:regmain}
\small
\input{output/tab/tab_M7_regression_main.tex}
\end{table}
...
\label{tab:env}
\small
\input{output/tab/tab_M0_environment_snapshot.tex}
\end{table}
...
Please refer to the \href{https://github.com/Jincan-LI-HUB/Applied-Statistic-HM1-Project-Report.git}{online hub} for the full file map.
