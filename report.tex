...
\label{tab:desc}
\small
\input{output/tab/tab_M4_finalWorth_describe.tex}
\end{table}
...
\textbf{Course-statistics reading:} This is exactly the situation in which the arithmetic mean is the wrong first summary. In the descriptive-statistics framework, the sample mean is sensitive to outliers, whereas the median, interquartile range, and trimmed mean are designed to describe the central mass under skewness. The report therefore treats the raw mean as evidence of tail dominance, not as the representative fortune of a typical billionaire; subsequent comparisons use log wealth, ranks, medians, and bootstrap intervals to keep the upper tail from silently controlling every conclusion.
...
\label{tab:top1}
\small
\input{output/tab/tab_M4_top1pct_wealth_share.tex}
\end{table}
...
\label{tab:gini}
\small
\input{output/tab/tab_M4_gini_verification.tex}
\end{table}
...
\label{tab:bca}
\small
\input{output/tab/tab_M4_bca_bootstrap.tex}
\end{table}
...
\textbf{Hypothesis-testing frame:} The primary tests use two-sided null hypotheses of no association or no group difference:
\[
H_0:\theta=0,\qquad H_1:\theta\neq 0,
\]
where $\theta$ is a correlation, mean contrast, rank contrast, or industry effect size depending on the row of Table~\ref{tab:infer}. Rejecting $H_0$ controls Type I error only for the specified test; it does not by itself establish a practically important effect. Conversely, failing to reject $H_0$ is not proof of equality. This is why the report pairs each $p$-value with an effect size, a confidence interval, and multiple-testing correction.

\textbf{Confidence-interval interpretation:} A 95\% confidence interval is interpreted in the repeated-sampling sense: under the stated procedure, intervals constructed over many comparable samples would cover the target parameter about 95\% of the time. It is not a statement that the fixed parameter has a 95\% probability of lying in the one interval observed here. Because the billionaire data are heavy-tailed and not plausibly bivariate normal, the correlation and effect-size intervals are bootstrap coverage approximations rather than normal-theory guarantees.
...
\label{tab:infer}
\small
\input{output/tab/tab_M6_inferential_results.tex}
\end{table}
\end{landscape}
...
\textbf{Why bootstrap rather than only Fisher's $z$:} The linear-regression course material motivates Fisher's $z$ interval for a population correlation under approximately bivariate Normal conditions. That approximation is not credible for billionaire wealth: even after logging, the joint distribution remains heavy-tailed and heterogeneous by industry and self-made status. The bootstrap CI is therefore used as the primary uncertainty summary, with Fisher's $z$ treated as a useful course benchmark rather than the governing model.
...
\label{tab:groupcorr}
\small
\input{output/tab/tab_M5_groupwise_correlations.tex}
\end{table}
...
\label{tab:posthoc}
\scriptsize
\input{output/tab/tab_M6_posthoc_category_pairwise.tex}
\end{table}
...
\resizebox{\textwidth}{!}{\input{output/tab/tab_M7_regression_main.tex}}
\end{table}
...
\label{tab:balance}
\small
\input{output/tab/tab_M7_sample_balance.tex}
\end{table}
...
\small
\input{output/tab/tab_M7_nested_ftest.tex}
\end{table}
...
\label{tab:kfoldcv}
\small
\input{output/tab/tab_M7_kfold_cv.tex}
\end{table}
...
\label{tab:quantreg}
\small
\input{output/tab/tab_M7_quantile_regression.tex}
\end{table}
...
\label{tab:vif}
\small
\input{output/tab/tab_M7_vif.tex}
\end{table}
...
\begin{table}[H]
\centering
\caption{Course regression-assumption audit for the primary log-log model. The course assumptions are used as a diagnostic checklist, not as a claim that the observational data satisfy an ideal experiment.}
\label{tab:course_reg_audit}
\small
\begin{tabularx}{\linewidth}{p{3.1cm}Y Y}
\toprule
Assumption & Evidence in this report & Treatment \\
\midrule
Linearity in parameters & Log-log scatter and fitted line show a weak approximately linear mean relation. & Interpret slope as elasticity; avoid causal language. \\
Independent errors & Dataset is cross-sectional at the person level, but country and industry clustering may remain. & Acknowledge omitted clustering; treat inference as descriptive association. \\
Zero mean conditional error & Cannot be verified directly because firm-level omitted variables are unavailable. & Emphasize unexplained variance and non-causal interpretation. \\
Constant variance & Residual plot and Breusch--Pagan test indicate heteroskedasticity. & Use HC3 robust standard errors throughout. \\
Normal errors & Q-Q plot shows heavier tails than Normal. & Use log outcome, bootstrap CIs, rank methods, and robust SE. \\
Low multicollinearity & VIF table flags severe collinearity among macro variables. & Keep GDP elasticity interpretable; treat Model C as controlled association, not a stable prediction model. \\
\bottomrule
\end{tabularx}
\end{table}
...
\label{tab:bp}
\small
\input{output/tab/tab_M7_bp_test_modelB.tex}
\end{table}
...
\label{tab:robtab}
\small
\input{output/tab/tab_M7_robust_drop_top1pct_logmodel.tex}
\end{table}
...
\resizebox{\textwidth}{!}{\input{output/tab/tab_M8_robustness_matrix.tex}}
\end{table}
...
\textbf{PCA and Bayesian extensions considered but not used as primary analyses.} Principal component analysis is relevant because the country-level macro variables are strongly collinear; a PCA score could compress GDP, CPI, education, life expectancy, and tax variables into a lower-dimensional development index. It is not used as the primary model because the research question asks for interpretable GDP elasticity, and replacing GDP with a component would make the main coefficient harder to explain. Bayesian inference is also a plausible future extension, especially a hierarchical country/industry model with partial pooling. The present dataset, however, contains one cross-sectional Forbes snapshot and no defensible external prior for individual fortunes, so a Bayesian model would add modeling assumptions without adding new observed information. Both methods are therefore noted as principled future work rather than forced into the main evidence chain.
...
\midrule
 ...
 & Course testing frame & $H_0/H_1$, Type I error, CI coverage, and Fisher-$z$ limits stated explicitly & \cellcolor{green!25} \\
 & Regression assumptions & Linearity, independence, zero-mean error, homoskedasticity, Normal error, and VIF audited & \cellcolor{green!25} \\
\midrule
 ...
 & PCA/Bayesian scope & Considered as course-relevant extensions, but excluded from the main chain to preserve interpretability & \cellcolor{yellow!25} \\
...
\label{tab:env}
\scriptsize
\resizebox{\textwidth}{!}{\input{output/tab/tab_M0_environment_snapshot.tex}}
\end{table}
...
\label{tab:runbook}
\small
\input{output/tab/tab_M9_runbook.tex}
\end{table}
...
\scriptsize
\resizebox{\linewidth}{!}{\input{output/tab/tab_M8_v2_integration_matrix.tex}}
\end{table}
\end{landscape}
